...
// code for command line args -- for now hard coding command line args		
		...
		String input;
			...
			else {input=args[0].toString().toUpperCase();}
		int length= input.length();
		System.out.println("Observation sequence Q:"+ " "+ input);
		...
		System.out.println("********************************");
		WriteToFile("********************************");
		System.out.println("cherries--"+cherrycount + "\n limes--"+ limecount);
	...
	private static void output(Double h1alpha, Double h2alpha, Double h3alpha,
		...
		System.out.println("P(h1 | Q) ="+String.format("%.5f",h1alpha));
		System.out.println("P(h2 | Q) ="+String.format("%.5f",h2alpha));
		System.out.println("P(h3 | Q) ="+String.format("%.5f",h3alpha));
		System.out.println("P(h4 | Q) ="+String.format("%.5f",h4alpha));
		System.out.println("P(h5 | Q) ="+String.format("%.5f",h5alpha));
		System.out.println("*******************************");
		double cherry=(h1*h1alpha+h2*h2alpha+h3*h3alpha+h4*h1alpha+h5*h1alpha);
		System.out.println("Probability that the next candy we pick will be C, given Q:"+String.format("%.5f",cherry));
		double lime=(l1*h1alpha+l2*h2alpha+l3*h3alpha+l4*h1alpha+l5*h1alpha);
		System.out.println("Probability that the next candy we pick will be L, given Q:"+String.format("%.5f",lime));
		//to file
